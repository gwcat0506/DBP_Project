...
const checkUser = async (req, res) => {
  ...
  if (!user_id)
    return res
      .status(400)
      .json({ status: 400, success: false, message: "아이디를 입력해주세요." });
  ...
  if (check)
    return res.status(400).json({
      status: 400,
      success: false,
      message: "이미 중복되는 아이디가 존재합니다.",
    });

  return res
    .status(200)
    .json({ status: 200, success: true, message: "사용 가능한 아이디입니다." });
};
...
const signupUser = async (req, res) => {
  ...
  if (!user_num || !user_id || !password)
    return res
      .status(400)
      .json({ status: 404, message: "모든 값을 입력해주세요." });
  ...
  try {
    if (check) {
      return res
        .status(200)
        .json({ status: 200, success: true, message: "회원가입 성공" });
    }
  } catch {
    return res
      .status(500)
      .json({ status: 500, success: false, message: "회원가입 실패" });
  }
};
...
const loginUser = async (req, res) => {
  ...
  if (!user_id || !password)
    return res
      .status(400)
      .json({ status: 404, message: "아이디와 비밀번호를 입력해주세요." });
